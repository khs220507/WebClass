...
<% int index = handler.getGrpStartPage();
 ...
 %>
     <a href="/prj0911_paging/list3.page?p=<%=index-1%>">[이전]</a> 
<%	   
   }
   while( index  <= handler.getGrpEndPage()){
%>
  <a href="/prj0911_paging/list3.page?p=<%=index %>" > [ <%=index %>]</a>
<%
	index++;
 } 
   if( handler.getGrpEndPage() < handler.getTotalPage()) {
%>
     <a href="/prj0911_paging/list3.page?p=<%=handler.getGrpEndPage()+1%>">다음</a>
<%} %>
